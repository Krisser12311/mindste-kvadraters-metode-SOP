...
Programmet kan som nævnt i det foregående afsnit, skrives på forskellige måder, afhængigt af hvilket paradigme, biblioteker og sprog der anvendes. Programmet her er skrevet i Python og anvender sumPy, NumPy og mathplotlib. Programmet er ikke optimeret på en måde hvor det fylder så lidt som muligt, samt så det kører så hurtigt som muligt. Det er derimod skrevet på en måde hvor det er nemt at forstå. Programmet er skrevet på en sådan måde at det nærmest er, en til en kontra Pseudokoden beskrevet i afsnit \ref{sec:Pseudokode}. Programmet består af tre funktioner. En funktion der hedder \pythoninline{leastSquaresMethod(dataPoints)}. Den funktion beregner $a$ og $b$. Den anden funktion hedder \pythoninline{plotter(dataPoints)} den funktion har til opgave at plottet beregningerne og punkterne ind i et koordinatsystem. Tilsidst har programmet en funktion der hedder \pythoninline{main()}. Funktionen \pythoninline{main()} styre hele programmet det er herfra de andre funktioner kaldes.  \\\\ Når programmet køres møder slut bruger et user interface. Dette interface spørger ind til tre ting. Den første mulighed er bare at indsætte valgfrie datapunkter, den anden muligehed er at anvende et forprogrammeret datasæt og den sidste muligehed er at lukke programmet. Det er en nødvendighed at have en exit funktion da funktionen \pythoninline{main()} er rekursiv og derfor kalder sig selv, i tilfælde at input fejl. Efter denne overordnet introduktion til programmets opbygning vil der nu blive set nærmere på, hvordan de tre centrale funktioner arbejder sammen for at implementere Mindste Kvadraters Metode. Hver funktion spiller en afgørende rolle i processen, og det er derfor relevant at gennemgå deres specifikke opgaver og sammenhæng. % Inkorporer FLOW CHART!
\subsubsection{Gemmengang af koden for Mindste Kvadraters Metode}
%I dette afsnit vil koden blive forklaret bid for bid. Først forklares funktion der finder $a$ og $b$ og derefter funktionen der plottet punkterne og linjen. Til sidst forklares hovedfunktionen der styrer det hele. 
%\begin{python}
